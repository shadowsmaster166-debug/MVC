...
class app{
    ...
    public function run(){
        ...
        foreach($this->handlers as $handler){
            ...
            if(str_contains(implode("/",$url), implode("/", $new_path))){
                ...
                if(implode('/',$new_path) == implode('/', $new_url) &&
        ...
        //     // Memproses path dari handler untuk dibandingkan dengan kurl
        //     $kpath = (isset($path[0]) ? $path[0] : '') . (isset($path[1]) ? '/' . $path[1] : '');

        //     // PERBAIKAN: Membandingkan kurl dengan kpath secara langsung di dalam loop
        //     if($kurl == $kpath && $handler['method'] == $requestMethod){
        ...
        //         // Mengambil method dari handler yang didaftarkan (Barang, index)
        //         if(isset($handler['handler'][1]) && method_exists($this->controllerFile, $handler['handler'][1])){
        //             $this->controllerMethod=$handler['handler'][1];
        //             unset($url[1]);
        //         }
        ...
        // Jika rute tidak ditemukan di loop atas, jalankan default
        if($execute==0){
            require_once __DIR__ .'/../controllers/'.$this->controllerFile.'.php';
            $this->controllerFile=new $this->controllerFile;
        }
    ...
    private function getUrl(){
        $url = isset($_SERVER['QUERY_STRING']) ? rtrim($_SERVER['QUERY_STRING'],'/') : '';
